...
import require$$0$2, { resolve, parse as parse$5, join } from 'path';
...
(function (sax) { // wrapper for non-node envs
  ...
  SAXStream.prototype.write = function (data) {
    if (typeof Buffer === 'function' &&
      ...
      if (!this._decoder) {
        var SD = require$$0$3.StringDecoder;
...
function getRoutes(options) {
  return [
    ...glob.sync("**/*.html", { cwd: options.outDir }).map((route) => {
      const parsedRoute = parse$5(route.replace(/index\.html/g, ""));
      return slash$1(join("/", parsedRoute.dir, parsedRoute.name));
    }),
    ...options.dynamicRoutes.map((route) => slash$1(join("/", join(parse$5(route).dir, parse$5(route).name))))
  ];
}
